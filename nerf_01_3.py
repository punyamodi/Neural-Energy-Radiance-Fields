...
if __name__ == '__main__':
    ...
    #testing_dataset = testing_dataset[:320000]    # Use first 160,000 samples for one test image

    # Create model with reduced complexity
    model = NerfModel(
        embedding_dim_pos=10,        # Reduced from 10 to 6 for faster computation
        embedding_dim_direction=4,  # Reduced from 4 to 2
        hidden_dim=256             # Reduced from 256 to 128
    ).to(device)
    ...
    training_loss = train(
        ...
        nb_epochs=1,               # Reduced number of epochs from int(1e5) to 1
        device=device, hn=2, hf=6,
        nb_bins=192,                # Reduced from 192 to 32
        H=400, W=400
    )
    ...
    H = 400
    W = 400
    num_images = len(testing_dataset) // (H * W)

    # Testing after training
    for img_index in range(num_images):
        test(
            hn=2, hf=6, dataset=testing_dataset,
            img_index=img_index, nb_bins=192,   # Consistent with training
            H=H, W=W
        )