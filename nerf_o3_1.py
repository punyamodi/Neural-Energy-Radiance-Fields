...
def test(hn, hf, dataset, chunk_size=10, img_index=0, nb_bins=192, H=400, W=400):
    ...
    """
    # Compute indices for the rays belonging to the view with index img_index.
    ...
    plt.imshow(img)
    plt.axis("off")
    plt.savefig(f'novel_views/img_{img_index}.png', bbox_inches='tight', pad_inches=0)
    plt.close()
...
def train(nerf_model, optimizer, scheduler, data_loader, device='cpu', hn=0, hf=1, nb_epochs=int(1e5),
    ...
    for epoch in range(nb_epochs):
        ...
        # If a testing dataset is provided, render all available test views.
        if testing_dataset is not None:
            # Compute the number of test views from the dataset size.
